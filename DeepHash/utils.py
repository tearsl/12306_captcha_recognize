...
def init_cifar_dataloader(root, batchSize):
    ...
    train_loader = DataLoader(CIFAR10(root, train=True, download=False, transform=transform_train),
                              batch_size=batchSize, shuffle=True, num_workers=0, pin_memory=True)
    print(f'train set: {len(train_loader.dataset)}')
    test_loader = DataLoader(CIFAR10(root, train=False, download=False, transform=transform_test),
                             batch_size=batchSize * 8, shuffle=False, num_workers=0, pin_memory=True)
